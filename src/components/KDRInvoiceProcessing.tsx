...
import { ChevronLeft, User, Trash2, LogOut, Paperclip, Send, FileText } from 'lucide-react';
...
export function KDRInvoiceProcessing({ onBack, onLogout, user }: KDRInvoiceProcessingProps) {
  ...
  const handleSend = async () => {
    if (message.trim() || attachments.length) {
      ...
      const MODULE: 'kdrInvoice' = 'kdrInvoice';
    ...
    <div className="min-h-screen flex flex-col">
      <header className="chat-header border-b border-[#2a3144] bg-[#0f1419]/50 backdrop-blur-md" style={{ height: 'var(--chat-header-height)' }}>
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" onClick={onBack} className="text-white hover:bg-[#1a1f2e]">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-white">KDRs Invoice Processing</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" className="text-white hover:bg-[#1a1f2e]" onClick={handleClearMessages}>
              <Trash2 className="w-5 h-5" />
            </Button>
            <Button variant="ghost" onClick={onLogout} className="text-white hover:bg-[#1a1f2e] gap-2">
      ...
      <main className="chat-main p-4">
        <div className="max-w-4xl mx-auto space-y-3">
          ...
          {messages.map((m) => (
            <div key={m.id} className="flex animate-fade-in" style={{ justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
              <div style={{ maxWidth: '75%' }}>
                <div className={m.role === 'user' ? "rounded-xl px-4 py-2 bg-gradient-to-r from-[#4A90F5] to-[#C74AFF] text-white animated-gradient" : "rounded-xl px-4 py-2 bg-[#1a1f2e]/80 border border-[#2a3144] text-white"}>
                  ...
                  {m.text && <div className="whitespace-pre-line">{renderTextWithLinks(m.text)}</div>}
      ...
      <footer className="chat-footer border-t border-[#2a3144] bg-[#0f1419]/50 backdrop-blur-md" style={{ height: 'var(--chat-footer-height)' }}>
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-3 max-w-4xl mx-auto">
            <Button variant="ghost" size="icon" onClick={handleAttachClick} className="text-gray-400 hover:bg-[#1a1f2e] shrink-0">
              <Paperclip className="w-5 h-5" />
            </Button>
            <Input placeholder="Type your message..." value={message} onChange={(e) => setMessage(e.target.value)} onKeyPress={handleKeyPress} className="flex-1 h-12 bg-[#1a1f2e] border-[#2a3144] text-white placeholder:text-gray-500" />
            <Button onClick={handleSend} disabled={!message.trim() && attachments.length === 0} className="shrink-0 bg-gradient-to-r from-[#4A90F5] to-[#C74AFF] hover:opacity-90 text-white animated-gradient disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {attachments.length > 0 && (
            <div className="attachments-list max-w-4xl mx-auto mt-3 flex flex-col gap-2">
              {attachments.map((a) => (
                <AttachmentItem key={a.id} name={a.file.name} size={a.file.size} type={a.file.type || 'application/octet-stream'} progress={a.progress} status={a.status} error={a.error} previewUrl={a.previewUrl} onDelete={() => {
                  if (a.cancel && a.status === 'uploading') a.cancel();
